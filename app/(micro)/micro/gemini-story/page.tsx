...
import { Loader2, AlertCircle, ImageIcon, ChevronLeft, ChevronRight, BookOpen } from "lucide-react"
...
export default function GeminiStory() {
  ...
  useEffect(() => {
    if (storyScenes.length > 0 && autoSlide) {
      ...
      autoSlideIntervalRef.current = setInterval(() => {
        setCurrentSceneIndex((prevIndex) => (prevIndex === storyScenes.length - 1 ? 0 : prevIndex + 1))
  ...
  async function onSubmitStory(values: z.infer<typeof storyFormSchema>) {
    ...
    try {
      ...
      if (!responseData.scenes || !Array.isArray(responseData.scenes) || responseData.scenes.length === 0) {
      ...
      console.log(
        `Received ${responseData.scenes.length} scenes, ${responseData.scenes.filter((s: StoryScene) => s.imageUrl).length} with images`,
      )
    ...
    <div className="min-h-screen bg-gradient-to-b from-slate-50 to-slate-100 dark:from-slate-900 dark:to-slate-800">
      ...
      <div className="bg-gradient-to-r from-purple-600 to-indigo-600 text-white py-12 px-4 sm:px-6 lg:px-8 mb-10">
        <div className="max-w-4xl mx-auto text-center">
          ...
          <h1 className="text-4xl font-extrabold tracking-tight sm:text-5xl md:text-6xl mb-4">Gemini Story Creator</h1>
          <p className="text-xl max-w-2xl mx-auto">
            Transform your ideas into captivating visual stories with AI-generated text and images
          </p>
        </div>
      </div>
      ...
      <div className="container mx-auto px-4 py-8">
        <Card className="max-w-2xl mx-auto shadow-lg border-purple-100 dark:border-purple-900 pt-0">
          <CardHeader className="p-4 bg-gradient-to-r from-purple-50 to-indigo-50 dark:from-purple-950/30 dark:to-indigo-950/30 rounded-t-lg">
            <CardTitle className="text-2xl text-purple-800 dark:text-purple-300">Create Your Story</CardTitle>
            <CardDescription className="text-purple-600 dark:text-purple-400">
              Describe your story idea and watch as Gemini brings it to life with vivid scenes and matching imagery.
            </CardDescription>
          </CardHeader>
            ...
            <Form {...storyForm}>
              <form onSubmit={storyForm.handleSubmit(onSubmitStory)} className="space-y-6">
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      <FormLabel className="text-lg font-medium">Story Prompt</FormLabel>
                      <FormControl>
                        <Textarea
                      ...
                      <FormDescription>
                        Be specific with characters, setting, and plot for more detailed and coherent stories.
                      </FormDescription>
                      <FormMessage />
                    </FormItem>
                  )}
        ...
        {storyScenes.length > 0 && (
          <div className="mt-16 max-w-6xl mx-auto">
            <div className="flex justify-between items-center mb-6">
              <h2 className="text-3xl font-bold text-purple-800 dark:text-purple-300">Your Story</h2>
                  ...
                  {storyScenes[currentSceneIndex].imageUrl ? (
                    <>
                      ...
                      <div className="md:w-2/3 h-[250px] md:h-full">
                        <img
                          src={storyScenes[currentSceneIndex].imageUrl || "/placeholder.svg"}
                      ...
                      <div className="md:w-1/2 h-[250px] md:h-full overflow-hidden">
                        <ScrollArea className="h-full p-6 md:p-8">
                          ...
                          <div className="prose dark:prose-invert prose-purple max-w-none">
                            <ReactMarkdown>{storyScenes[currentSceneIndex].text}</ReactMarkdown>
                          </div>
                        </ScrollArea>
                      </div>
                    </>
            ...
            <details className="mt-8 border rounded-lg p-4 bg-slate-50 dark:bg-slate-800/30 dark:border-slate-700">
              <summary className="font-medium cursor-pointer text-purple-700 dark:text-purple-400">
                Show Raw Response Data
              </summary>
              <div className="mt-4 bg-slate-900 text-emerald-400 p-4 rounded overflow-auto max-h-96">
                <pre>{rawStoryResponse}</pre>
              </div>
            </details>
          </div>
        )}
